...
RUN cd root/ && wget --continue https://ftp.postgresql.org/pub/source/v8.4.22/postgresql-8.4.22.tar.gz && tar -xzf postgresql-8.4.22.tar.gz
RUN cd /root/postgresql-8.4.22 && ./configure --prefix=/var/lib/postgresql && make && make install
